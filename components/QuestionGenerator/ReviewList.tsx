    ...
    <div className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100 h-full flex flex-col animate-fade-in-up">
      <header className="flex justify-between items-center mb-8 pb-6 border-b border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm">Toàn bộ câu hỏi sẽ được lưu vào: <span className="text-blue-600 font-bold">{folderName}</span></p>
        </div>
        <div className="flex gap-3">
          <button onClick={onCancel} className="px-6 py-2.5 text-gray-400 font-bold hover:text-gray-600">Hủy bỏ</button>
          <button onClick={onApproveAll} className="px-8 py-2.5 bg-blue-600 text-white rounded-xl font-bold shadow-xl hover:bg-blue-700 transition">
        ...
        {questions.map((q, i) => (
          <div key={q.id} className="p-8 bg-gray-50/50 rounded-[2.5rem] border border-gray-100 relative group overflow-hidden">
             ...
             <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-[10px] font-black text-gray-400 bg-white px-3 py-1 rounded-full border border-gray-100 mr-12">{q.bloomLevel}</span>
             </div>
